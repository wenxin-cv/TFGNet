...
from models.basicsr.losses import build_loss
from models.basicsr.metrics import calculate_metric
from models.basicsr.utils import get_root_logger, imwrite, tensor2img
...
class JointModel(SRModel):
    ...
    def init_training_settings(self):
        ...
        if self.ema_decay > 0:
            ...
            from models.basicsr.archs import build_network
    ...
    def optimize_parameters(self, current_iter):
        ...
        if current_iter <= 40000:
            small_per_n = 1
            w = 1
        elif current_iter <= 80000:
            small_per_n = 1
            w = 1.3
        elif current_iter <= 120000:
            small_per_n = 120000
            w = 0
        ...
        if current_iter % small_per_n == 0:
            from models.basicsr.archs import build_network
            self.net_g = build_network(self.opt['network_g']).to(self.device)
            self.net_g.train()
